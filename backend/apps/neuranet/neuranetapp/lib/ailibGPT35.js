...
exports.process = async function(data, promptOrPromptFile, apiKey, model, dontInflatePrompt) {
    ...
    do {    // auto retry if API is overloaded (503 error)
        ...
        await rest.postHttps(modelObject.driver.host, modelObject.driver.port, modelObject.driver.path,modelObject?.model_type =="llama"? 
        {"Authorization": `Basic ${apiKey}`,"x-api-key":modelObject.x_api_key}:{"Authorization": `Bearer ${apiKey}`}, promptObject); 
        retries++;
