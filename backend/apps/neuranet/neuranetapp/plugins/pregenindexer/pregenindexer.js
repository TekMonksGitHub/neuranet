...
async function _getPregenStepsAIApp(fileindexer) {
    ...
    const pregenFunctions = []; for (const pregenStepObject of pregenStepObjects) {
        const genfilesDir = fileindexer.systemFilesPath, generateFilesExt = fileindexer.systemFilesExtension,
            cmspath = `${path.dirname(fileindexer.cmspath)}/${genfilesDir}/${pregenStepObject.in.pathid}_${path.basename(fileindexer.cmspath)}.${generateFilesExt}`,
            cmspathTo = fileindexer.cmspathTo ? `${path.dirname(fileindexer.cmspathTo)}/${genfilesDir}/${pregenStepObject.in.pathid}_${path.basename(fileindexer.cmspathTo)}.${generateFilesExt}` : undefined,
